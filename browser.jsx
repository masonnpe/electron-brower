...
var BrowserChrome = React.createClass({
  ...
  locationContextMenu: function (el) {
    var self = this
    var menu = new Menu()
    menu.append(new MenuItem({ label: '剪切', click: function () {
  ...
  webviewContextMenu: function (e) {
    ...
    var menu = new Menu()
    if (e.href) {
      menu.append(new MenuItem({ label: '在新标签页中打开链接', click: function () { self.createTab(e.href) } }))
    ...
    if (e.img) {
      ...
      menu.append(new MenuItem({ label: '在新标签页中打开图片', click: function () { self.createTab(e.img) } }))
    }
    if (e.hasSelection)
      menu.append(new MenuItem({ label: '复制', click: function () { self.getWebView().copy() } }))
      menu.append(new MenuItem({ label: '全选', click: function () { self.getWebView().selectAll() } }))
      menu.append(new MenuItem({ label: '审查元素', click: function() { self.getWebView().inspectElement(e.x, e.y) } }))
      menu.popup(remote.getCurrentWindow())
  },
  ...
  newWindowOpen:function(e){
    this.createTab(e.url)
  },
  goLink:function(e,webview){
